...
export function updateBossAttacks(game, dt) {
  ...
  for (const p of game.enemyProjectiles) if (p.seekRemaining > 0) {
    p.seekRemaining -= dt;
    const desired = Math.atan2(game.player.y - p.y, game.player.x - p.x);
  ...
  game.hazards = game.hazards.filter(h => {
    ...
    if (Math.hypot(game.player.x - h.x, game.player.y - h.y) < h.radius + game.player.size) game.player.takeDamage(24, game);
