...
export class BulletGame {
  ...
  special() {
    if (this.state !== 'playing' || this.time < this.specialReadyAt) return false;
  ...
  updateWaves(dt) {
    ...
    if (this.wave.breather) return;
    this.spawnTimer -= dt;
    if (this.spawnTimer > 0) return;
    const difficulty = difficultyAt(this.time);
    this.spawnTimer = Math.max(1.4, this.wave.interval * difficulty.spawnInterval / 1.1);
    const count = this.time === 0 ? 5 : Math.min(12, this.wave.count * difficulty.batch);
    ...
    for (let i = 0; i < count; i++) {
      ...
      const key = this.wave.pool[(i + this.packet) % this.wave.pool.length];
  ...
  collectDeaths() {
    ...
    for (const e of dead) {
      ...
      if (e.splitter && e.type.splitInto) {
        ...
        if (def) for (let i = 0; i < (e.type.splitCount || 2) && this.enemies.length < CONFIG.MAX_ENEMIES; i++) {
          const difficulty = difficultyAt(this.time);
  ...
  snapshot() {
    ...
    return {
      wave: { number: this.wave.number, title: this.wave.title, breather: this.wave.breather, secondsLeft: this.wave.secondsLeft },
