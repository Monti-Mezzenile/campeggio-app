...
import { levelXpCost, NORMAL_PICKUP_CHANCE, availableUpgradeTier } from './progression.js';
import { bulletReward } from '../game-rewards.js';
...
export const rewardForRun = bulletReward;
const noop = () => {};
...
export class BulletGame {
  ...
  reset() {
    ...
    this.player.levelXpCost = level => levelXpCost(level);
    this.player.expToNext = this.player.levelXpCost(1);
  ...
  spawn(def, { side, anchor } = {}) {
    ...
    let candidates = this.covered.filter(p => (this.players ?? [this.player]).every(player => Math.hypot(p.x - player.x, p.y - player.y) > 300));
    if (side) {
  ...
  collectDeaths() {
    ...
    for (const e of dead) {
      ...
      if (e.boss || this.random() < NORMAL_PICKUP_CHANCE) {
  ...
  offerUpgrades() {
    const tierLimit = availableUpgradeTier(this.bossKills);
    const pool = [];
    for (const [id, [label]] of Object.entries(WEAPON_LABELS)) {
      const owned = this.player.weapons.find(w => w.id === id);
      if ((!owned && this.player.weapons.length < CONFIG.MAX_WEAPONS) || (owned && owned.level < tierLimit)) {
    ...
    for (const [id, [label]] of Object.entries(PASSIVE_LABELS)) {
      ...
      if (count < tierLimit) pool.push({ id, label, description: upgradeDescription(id, count + 1), kind: 'passive', level: count + 1, maxLevel: UPGRADE_CAP });
    }
  ...
  update(dt) {
    ...
    if (this.updatePlayers) this.updatePlayers(dt);
    else {
    ...
    for (const enemy of this.enemies) {
      ...
      const restore = this.selectTarget?.(enemy);
    ...
    for (const p of this.enemyProjectiles) {
      const restore = this.selectTarget?.(p); p.update(dt, this); restore?.();
    }
    ...
    this.pickups = this.pickups.filter(p => {
      p.life -= dt;
      const target = this.nearestPlayer?.(p) ?? this.player;
      if (Math.hypot(p.x - target.x, p.y - target.y) < 30) {
        const restore = this.selectTarget?.(p); this.applyPickup(p.kind); restore?.(); return false;
      }
      return p.life > 0;
    });
    ...
    if (this.teamDefeated ? this.teamDefeated() : this.player.dead) this.finish();
  ...
  snapshot() {
    ...
    return {
      ...
      maxUpgradeTier: availableUpgradeTier(this.bossKills),
