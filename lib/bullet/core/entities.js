...
export class Player {
    ...
    update(dt, game) {
        ...
        for (const w of this.weapons) {
            if (!game.recovering || w.def.type === 'orbit') w.update(dt, this, game);
            else w.cooldown = Math.max(0, w.cooldown - dt);
        }
    ...
    gainExp(amount) {
        ...
        while (this.exp >= this.expToNext) {
            ...
            this.level++;
            this.expToNext = this.levelXpCost ? this.levelXpCost(this.level) : Math.min(this.expThresholdCap ?? Infinity, Math.floor(this.expToNext * (this.expGrowth ?? 1.2)));
...
export class Enemy {
    ...
    update(dt, game) {
        ...
        if (this.illusionist && !this.isClone) {
            this.cloneTimer -= dt;
            if (this.cloneTimer <= 0 && game.enemies.length < (game.enemyLimit ?? CONFIG.MAX_ENEMIES)) {
                this.cloneTimer = this.type.cloneCooldown || 5.5;
                const n = this.type.cloneCount || 2;
                for (let i = 0; i < n && game.enemies.length < (game.enemyLimit ?? CONFIG.MAX_ENEMIES); i++) {
    ...
    update(dt, game) {
        ...
        if (this.boomerang) {
            ...
            if (d > this.maxDist * 0.5) {
                const ra = Math.atan2((this.owner ?? game.player).y - this.y, (this.owner ?? game.player).x - this.x);
        ...
        if (this.boomerang) {
            const dp = Math.hypot(this.x - (this.owner ?? game.player).x, this.y - (this.owner ?? game.player).y);
...
export class OrbitShard {
    ...
    update(dt, player, game) {
        ...
        if (game.recovering) return; // Keep orbit motion alive without hurting resting bosses.
...
export class ExpOrb {
    ...
    update(dt, game) {
        ...
        if (d < CONFIG.PICKUP_DISTANCE) {
            (game.progressPlayer ?? p).gainExp(this.value);
