...
export class Weapon {
    ...
    levelUp() {
        this.level = Math.min(this.def.maxLevel ?? Infinity, this.level + 1);
    }
    ...
    getRange(player) {
        if (this.def.ranges) return this.def.ranges[this.level - 1] * player.getAreaMult();
        return this.def.baseRange * (1 + (this.level - 1) * 0.1) * player.getAreaMult();
    }

    getOrbitShardCount(player) {
        if (this.def.orbitCounts) return this.def.orbitCounts[this.level - 1];
    ...
    _fireProjectile(player, game) {
        let count = this.def.projectileCounts?.[this.level - 1] ?? this.def.projectileCount + Math.floor((this.level - 1) / 2);
    ...
    _fireMine(player, game) {
        ...
        const count = this.def.mineCounts?.[this.level - 1] ?? 1;
        for (let i = 0; i < count; i++) {
            const angle = i * Math.PI * 2 / count;
            const offset = count > 1 ? 45 : 0;
            game.mines.push(new Mine(player.x + Math.cos(angle) * offset, player.y + Math.sin(angle) * offset, radius, dmg, fuse));
        }
        if (this.isEvolved()) {
    ...
    _fireNova(player, game) {
        ...
        const slowDur = this.def.slowDurations?.[this.level - 1] ?? this.def.slowDuration ?? 1.2;
