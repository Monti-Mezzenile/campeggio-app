...
export class SpatialHash {
    ...
    queryRect(x, y, r) {
        ...
        const out = [];
        // Homing projectiles ask for the nearest enemy over the whole arena.
        // Scan occupied buckets for broad queries instead of visiting ~100k
        // empty cells per projectile (the upstream 9999px search radius).
        if ((x1 - x0 + 1) * (y1 - y0 + 1) > this.map.size) {
            for (const bucket of this.map.values()) {
                const bx = Math.floor(bucket[0].x / c);
                const by = Math.floor(bucket[0].y / c);
                if (bx >= x0 && bx <= x1 && by >= y0 && by <= y1) out.push(...bucket);
            }
            return out;
        }
