...
export function renderGame(ctx, game, images, width, height) {
  ...
  const viewPlayer = players[game.viewIndex ?? 0];
  const cameraX = Math.max(0, Math.min(WORLD.width - width, viewPlayer.x - width / 2));
  const cameraY = Math.max(0, Math.min(WORLD.height - height, viewPlayer.y - height / 2));
  ...
  for (const player of players) for (const w of player.weapons) w.renderExtras(ctx);
  ...
  const actors = [...game.enemies, ...players].sort((a, b) => a.y - b.y);
  for (const actor of actors) {
    const isPlayer = players.includes(actor);
    ...
    if (isPlayer) {
      ...
      const context = game.contexts?.[index] ?? game;
      const move = context.input.getMoveVector();
      if (actor.dead) ctx.globalAlpha = 0.35;
      sprite(ctx, index === 1 ? images.playerBlue : images.player, playerFrame(game.time, context.specialAt, context.shootingPose, !actor.dead && Math.hypot(move.x, move.y) > 0.01), actor.x, actor.y, size);
      if (players.length > 1) {
        ctx.globalAlpha = 1; ctx.fillStyle = index === 1 ? '#7dd3fc' : '#f9a8d4';
        ctx.font = 'bold 12px sans-serif'; ctx.textAlign = 'center';
        ctx.fillText(actor.dead ? 'RIANIMA' : index === (game.viewIndex ?? 0) ? 'TU' : 'COMPAGNO', actor.x, actor.y - 42);
      }
    } else {
  ...
  for (let i = 0; i < players.length; i++) {
    const context = game.contexts?.[i] ?? game;
    if (context.shieldUntil > game.time) {
      ctx.strokeStyle = '#a5e8ff'; ctx.lineWidth = 3;
      ctx.beginPath(); ctx.arc(players[i].x, players[i].y, 38, 0, Math.PI * 2); ctx.stroke();
    }
    drawShockwave(ctx, context.specialOrigin, game.time - context.specialAt, game.reducedMotion);
  }
