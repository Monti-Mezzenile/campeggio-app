...
export default function BulletHell({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const saveRun = async (run: BulletSnapshot) => {
      ...
      try {
        ...
        if (!abort.signal.aborted) setReward('Connessione interrotta: salvataggio XP non confermato.');
      }
    };
    ...
    void mountBulletGame(canvas.current!, root.current!, receive, abort.signal, joystick.current!).then(game => {
  ...
  const start = () => { if (!muted) playMusic(true); finished.current = false; setReward(''); controller.current?.start(); };
  ...
  return <section ref={root} className={styles.game} aria-label="Bullet Hell: la rivolta della cavia">
    {snapshot && state === 'ready' && <div className={`${styles.overlay} ${styles.intro}`}>
      ...
      <button type="button" className={styles.primary} onClick={start}>GIOCA <span aria-hidden="true">→</span></button>
      <div className={styles.menuTools}>
        ...
        <button type="button" onClick={onClose}>← Cavia</button>
      </div>
    </div>}
    ...
    <div className={styles.arena}>
      ...
      {error && <div className={styles.overlay} role="alert"><h3>Il bosco non si apre</h3><p>{error}</p><button type="button" onClick={() => { setError(''); setLoadingAttempt(n => n + 1); }}>Riprova</button><button type="button" onClick={onClose}>Torna alla cavia</button></div>}
      {snapshot && state === 'paused' && <div className={styles.overlay}>
        ...
        <button type="button" className={styles.primary} onClick={togglePause}>Riprendi la partita</button>
